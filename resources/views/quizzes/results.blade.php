                    ...
                    <table class="table mb-0 align-items-center justify-content-center">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <p class="mb-0 text-sm font-weight-bold">{{ $attempt->score }} /
                                ...
                                <td>
                                    ...
                                    :
                                    ($attempt->score >= ($attempt->total_questions/2) ? 'bg-gradient-info' :
                                    'bg-gradient-danger');
                                    $statusText = $attempt->score == $attempt->total_questions ? 'Aprobado' :
                                    ($attempt->score >= ($attempt->total_questions/2) ? 'Intermedio' : 'Reprobado');
                                    @endphp
                                ...
                                <td>
                                    <p class="mb-0 text-sm font-weight-bold">
                                        {{ $attempt->completed_at->format('d/m/Y') }}</p>
                                </td>
                                <td class="align-middle">
                                    @if($attempt->completed_at) {{-- Mostrar botón si el quiz fue completado --}}
                                    <button class="mb-0 btn btn-sm btn-outline-info" data-bs-toggle="modal"
...
<div class="modal fade" id="detailsModal{{ $attempt->id }}" tabindex="-1"
    aria-labelledby="detailsModalLabel{{ $attempt->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="detailsModalLabel{{ $attempt->id }}">Detalles del Quiz:
                    {{ $attempt->quiz->title }}</h5>
            ...
            <div class="modal-body">
                <div class="mb-4">
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <p class="mb-1"><strong>Puntaje:</strong>
                                {{ $attempt->score }}/{{ $attempt->total_questions }}</p>
                        </div>
                        <!--<div class="col-md-4">
                            <p class="mb-1"><strong>Tiempo:</strong> {{ gmdate('i:s', $attempt->time_spent) }} min</p>
                        </div>-->
                        <div class="col-md-4">
                            <p class="mb-1"><strong>Fecha:</strong> {{ $attempt->completed_at->format('d/m/Y') }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="p-3 rounded"> {{-- Fondo amarillo para la pregunta --}}
                        <p class="mb-1 fw-bold text-dark">Pregunta {{ $index + 1 }}: {{ $detail['question'] }}</p>

                        <p class="mb-1">
                            Tu respuesta:
                            <span class="{{ $detail['is_correct'] ? 'text-success fw-bold' : 'text-danger fw-bold' }}">
                                {{ $detail['user_answer'] ?? 'No respondida' }}
                            </span>
                        </p>
                        ...
                        <p class="mb-0">
                            Respuesta correcta:
                            <span class="text-success fw-bold">{{ $detail['correct_answer'] }}</span>
                        </p>
                        @endif
                    </div>
                    @endforeach
